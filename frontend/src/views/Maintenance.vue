    ...
    <div class="max-w-2xl w-full text-center space-y-8 z-10">
      ...
      >
        <div class="glass-card rounded-2xl p-4 flex flex-col items-center justify-center">
          ...
          <div class="text-[10px] md:text-xs uppercase tracking-widest text-slate-500 mt-1 font-semibold">
            Days
          </div>
        </div>
        <div class="glass-card rounded-2xl p-4 flex flex-col items-center justify-center">
      ...
      <div class="pt-16 pb-4">
        <div class="inline-flex items-center space-x-3 text-sm text-slate-500 bg-slate-900/50 rounded-full px-4 py-2 border border-slate-800">
          ...
          <span class="font-medium tracking-wide">Our team is working hard behind the scenes.</span>
        </div>
      </div>
    </div>
  </div>
...
useHead({
  title: computed(() => `${title.value} | ${publicSettings.value?.site_name || 'System'}`)
});
