<template>
  ...
  >
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        <DialogTitle>{{ $t('modules.core.media.modals.folder.title') }}</DialogTitle>
        <DialogDescription>
          {{ $t('modules.core.media.modals.folder.placeholder') || 'Create a new folder to organize your media.' }}
        </DialogDescription>
      </DialogHeader>
      ...
      >
        <div class="grid gap-2">
          <label class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            {{ $t('modules.core.media.modals.folder.name') }} <span class="text-destructive">*</span>
          </label>
          <Input
            v-model="form.name"
            type="text"
            required
            :placeholder="$t('modules.core.media.modals.folder.placeholder')"
          />
        </div>

        <div class="grid gap-2">
          <label class="text-sm font-medium leading-none">
            {{ $t('modules.core.media.modals.folder.parent') }}
          </label>
          <Select 
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="none">
                {{ $t('modules.core.media.modals.folder.noParent') }}
              </SelectItem>
              <SelectItem
      ...
      <DialogFooter>
        <Button
          ...
          @click="$emit('close')"
        >
          {{ $t('modules.core.media.actions.cancel') }}
        </Button>
        <Button
        ...
        >
          ...
          />
          {{ saving ? $t('modules.core.media.modals.folder.creating') : $t('modules.core.media.modals.folder.create') }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
