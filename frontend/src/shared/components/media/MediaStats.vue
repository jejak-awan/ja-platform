  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <CardContent class="p-4">
        <div class="flex items-center justify-between space-y-0 pb-1">
          <p class="text-xs font-medium text-muted-foreground">
            {{ $t('modules.core.media.stats.total') }}
          </p>
    ...
    <Card>
      <CardContent class="p-4">
        <div class="flex items-center justify-between space-y-0 pb-1">
          <p class="text-xs font-medium text-muted-foreground">
            {{ $t('modules.core.media.stats.storage') }}
          </p>
    ...
    <Card>
      <CardContent class="p-4">
        <div class="flex items-center justify-between space-y-0 pb-1">
          <p class="text-xs font-medium text-muted-foreground">
            {{ $t('modules.core.media.stats.images') }}
          </p>
          <ImageIcon class="h-3.5 w-3.5 text-muted-foreground/50" />
        </div>
        <div>
          <div class="text-xl font-bold">
            {{ stats?.types?.find((t) => t.type === 'image')?.count || 0 }}
          </div>
          <p class="text-[10px] text-muted-foreground/70">
            {{ $t('modules.core.media.stats.images') }}
          </p>
    ...
    <Card>
      <CardContent class="p-4">
        <div class="flex items-center justify-between space-y-0 pb-1">
          <p class="text-xs font-medium text-muted-foreground">
            {{ $t('modules.core.media.stats.videos') }}
          </p>
          <VideoIcon class="h-3.5 w-3.5 text-muted-foreground/50" />
        </div>
        <div>
          <div class="text-xl font-bold">
            {{ stats?.types?.find((t) => t.type === 'video')?.count || 0 }}
          </div>
          <p class="text-[10px] text-muted-foreground/70">
            {{ $t('modules.core.media.stats.videos') }}
          </p>
...
import type { MediaStats } from '@/modules/Cms/types/cms';
