    ...
    >
      ...
      <div class="p-3 border-b border-border/40 flex items-center justify-between bg-transparent shrink-0">
        <h3 class="font-semibold text-xs uppercase tracking-widest text-muted-foreground/80">
          {{ $t('modules.core.media.modals.edit.title') }}
        </h3>
        <Button
          variant="ghost"
          size="icon"
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-3 p-3 bg-muted/30 rounded-lg border border-border/20 text-xs">
          <div>
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.core.media.modals.edit.size') }}</span>
            <span class="font-medium text-foreground">{{ formatFileSize(activeMedia.size) }}</span>
          </div>
          <div class="truncate">
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.core.media.modals.edit.type') }}</span>
            <span
          ...
          <div class="col-span-2 truncate">
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.core.media.modals.edit.folder') }}</span>
        ...
        >
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.core.media.modals.edit.name') }}
            </label>
            <Input
              v-model="form.name"
              type="text"
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.core.media.modals.edit.altText') }}
            </label>
            <Input
              v-model="form.alt"
              type="text"
              :placeholder="$t('modules.core.media.modals.edit.altPlaceholder')"
              class="bg-background h-8 text-xs"
            />
          </div>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.core.media.modals.edit.caption') || 'Caption' }}
            </label>
            <Input
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.core.media.modals.edit.description') }}
            </label>
            <textarea
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.core.media.modals.edit.tags') || 'Tags' }}
            </label>
          ...
          >
            ...
            <div class="flex-1">
              ...
              >
                {{ $t('modules.core.media.modals.edit.isShared') }}
              </label>
              <p class="text-[10px] text-muted-foreground mt-0.5 leading-snug">
                {{ $t('modules.core.media.modals.edit.isSharedHelp') }}
              </p>
            </div>
          </div>
          ...
          <div class="pt-3 sticky bottom-0 pb-2 mt-4 bg-transparent">
            <div class="flex gap-2">
              ...
              >
                {{ saving ? $t('modules.core.media.modals.edit.saving') : $t('modules.core.media.actions.save') }}
              </Button>
              <Button 
                type="button"
                variant="outline"
                ...
                :title="$t('modules.core.media.actions.copyUrl')"
                @click="copyUrl"
              >
                <Link class="w-4 h-4" />
              </Button>
              <Button 
                ...
                size="icon"
                class="shrink-0 h-9 w-9"
                :title="$t('modules.core.media.actions.download')"
      ...
      >
        ...
        <p class="text-sm font-medium">
          {{ $t('modules.core.media.modals.edit.select_item') || 'Select item to view details' }}
        </p>
      </div>
    </div>
  </div>
...
const getFolderName = (id?: number | null) => {
    if (!id) return t('modules.core.media.modals.edit.noFolder');
    const folder = folders.value.find(f => f.id === id);
    return folder ? folder.name : t('modules.core.media.modals.edit.noFolder');
};
...
const copyUrl = async () => {
    if (activeMedia.value?.url) {
        try {
            ...
            toast.success.action(t('modules.core.media.toast.urlCopied'));
        } catch (_err) {
...
const handleSubmit = async () => {
    ...
    try {
        ...
        toast.success.action(t('modules.core.media.messages.updateSuccess'));
    } catch (error) {
