      ...
      <div class="bg-card border border-border/40 shadow-none rounded-xl max-w-2xl w-full">
        <div class="flex items-center justify-between p-6 border-b border-border/40">
          <h3 class="text-lg font-semibold">
            {{ $t('modules.core.media.modals.resize.title') }}
          </h3>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div class="p-6 space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-foreground mb-1">
                {{ $t('modules.core.media.modals.resize.width') }}
              </label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-foreground mb-1">
                {{ $t('modules.core.media.modals.resize.height') }}
              </label>
              <input
          ...
          <div class="flex items-center">
            ...
            >
              {{ $t('modules.core.media.modals.resize.maintainAspectRatio') }}
            </label>
          </div>
          ...
          <div
            ...
            class="mt-4"
          >
            <label class="block text-sm font-medium text-foreground mb-2">{{ $t('modules.core.media.modals.resize.preview') }}</label>
        ...
        <div class="flex items-center justify-end space-x-3 p-6 border-t border-border/40">
          <Button
            ...
            @click="$emit('close')"
          >
            {{ $t('modules.core.media.modals.resize.cancel') }}
          </Button>
          <Button
            :disabled="resizing || !isValid"
            @click="handleResize"
          >
            {{ resizing ? $t('modules.core.media.modals.resize.resizing') : $t('modules.core.media.modals.resize.resize') }}
          </Button>
        </div>
      </div>
    </div>
...
const handleResize = async () => {
    ...
    try {
        ...
        toast.success.action(t('modules.core.media.modals.resize.success') || 'Image resized successfully');
        emit('resized');
    } catch (error: unknown) {
