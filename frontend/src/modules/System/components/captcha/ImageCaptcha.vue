  ...
  <div class="captcha-image w-full">
    ...
    >
      ...
      <input
        ...
        :placeholder="t('modules.system.auth.captcha.typeCode')"
        :disabled="verified"
        maxlength="6"
        autocomplete="off"
        @keyup.enter="verify"
      ...
      <div class="flex items-center gap-0.5 pr-1 h-full flex-none">
        ...
        <Button 
          ...
          :title="t('modules.system.auth.captcha.refresh')"
          @click="refresh"
        >
...
const verify = async () => {
    ...
    try {
        ...
        if (err.response && err.response.status === 422) {
            error.value = t('modules.system.auth.captcha.error') || 'Incorrect answer'
