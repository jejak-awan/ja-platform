    ...
    <div class="mb-10 flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-foreground">
          {{ $t('modules.system.users.title') }}
        </h1>
        <p class="text-muted-foreground">
          {{ $t('modules.system.users.subtitle') }}
        </p>
      </div>
      <router-link
      ...
      >
        <Button class="gap-2 rounded-xl">
          <Plus class="w-4 h-4" />
          {{ $t('modules.system.users.createNew') }}
        </Button>
      </router-link>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
        ...
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-muted-foreground">
              {{ $t('modules.system.users.stats.total') }}
            </p>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-muted-foreground">
              {{ $t('modules.system.users.stats.verified') }}
            </p>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-muted-foreground">
              {{ $t('modules.system.users.stats.unverified') }}
            </p>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-muted-foreground">
              {{ $t('modules.system.users.stats.recent') }}
            </p>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-muted-foreground">
              {{ $t('modules.system.users.stats.active') }}
            </p>
    ...
    <Card class="overflow-hidden rounded-xl border-border/50 shadow-sm">
          ...
          <div class="flex items-center gap-3 w-full md:w-auto flex-wrap">
            <div class="relative w-full md:w-72">
              ...
              <Input
                v-model="search"
                type="text"
                :placeholder="$t('modules.system.users.search')"
                class="pl-9"
              />
            </div>
            <Select v-model="roleFilter">
              <SelectTrigger class="w-[160px]">
                <SelectValue :placeholder="$t('modules.system.users.allRoles')" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {{ $t('modules.system.users.allRoles') }}
                </SelectItem>
                <SelectItem
                  v-for="role in roles"
                  :key="role.id"
            ...
            <Select v-model="verificationFilter">
              <SelectTrigger class="w-[160px]">
                <SelectValue :placeholder="$t('modules.system.users.filters.all')" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {{ $t('modules.system.users.filters.all') }}
                </SelectItem>
                <SelectItem value="verified">
                  {{ $t('modules.system.users.filters.verifiedOnly') }}
                </SelectItem>
                <SelectItem value="unverified">
                  {{ $t('modules.system.users.filters.unverifiedOnly') }}
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            >
              ...
              >
                ...
                <SelectContent>
                  ...
                  >
                    {{ $t('modules.system.users.actions.forceLogout') }}
                  </SelectItem>
                  <SelectItem
                    value="verify"
                    class="text-primary focus:text-primary"
                  >
                    {{ $t('modules.system.users.actions.verify') }}
                  </SelectItem>
                  <SelectItem
      ...
      <CardContent class="p-0">
        <DataTable
          :table="table"
          :loading="loading"
          :empty-message="$t('modules.system.users.empty')"
        />
...
const columns = [
    ...
    columnHelper.accessor('name', {
        header: t('modules.system.users.table.user'),
        cell: ({ row }) => {
    ...
    columnHelper.accessor('email', {
        header: t('modules.system.users.table.email'),
        cell: ({ row }) => {
            const user = row.original;
            return h('div', [
                h('div', { class: 'text-sm text-foreground' }, user.email),
                user.email_verified_at 
                    ? h('div', { class: 'text-[10px] text-primary font-bold uppercase tracking-wider' }, t('modules.system.users.status.verified'))
                    : h('div', { class: 'text-[10px] text-muted-foreground italic uppercase tracking-wider' }, t('modules.system.users.status.unverified'))
            ]);
        }
    }),
    columnHelper.accessor('roles', {
        header: t('modules.system.users.table.roles'),
        cell: ({ row }) => {
            const roles = row.original.roles || [];
            if (roles.length === 0) return h('span', { class: 'text-xs text-muted-foreground italic' }, t('modules.system.users.status.noRoles'));
    ...
    columnHelper.accessor('last_login_at', {
        header: t('modules.system.users.table.lastLogin'),
        cell: ({ row }) => {
            const date = row.original.last_login_at;
            if (!date) return h('div', { class: 'text-xs text-muted-foreground/50' }, t('modules.system.users.status.never'));
            return h('div', { class: 'text-xs' }, formatDate(date));
        }
    }),
    columnHelper.display({
        id: 'actions',
        header: () => h('div', { class: 'text-center' }, t('modules.system.users.table.actions')),
        cell: ({ row }) => {
            const user = row.original;
            ...
            return h('div', { class: 'flex justify-center items-center gap-1' }, [
                !user.email_verified_at && authStore.hasPermission('edit users') && h(Button, {
                    ...
                    disabled: !canManageUser, onClick: () => verifyUser(user), title: t('modules.system.users.actions.verify')
                ...
                authStore.hasPermission('edit users') && h(Button, {
                    ...
                    disabled: !canManageUser, onClick: () => forceLogoutUser(user), title: t('modules.system.users.actions.forceLogout')
...
const deleteUser = async (user: User) => {
    const confirmed = await confirm({
        ...
        message: t('modules.system.users.messages.deleteConfirm', { name: user.name }),
        variant: 'danger',
...
const forceLogoutUser = async (user: User) => {
    const confirmed = await confirm({
        title: t('modules.system.users.actions.forceLogout'),
        message: t('modules.system.users.messages.forceLogoutConfirm', { name: user.name }),
        variant: 'warning',
        confirmText: t('modules.system.users.actions.forceLogout'),
    });
...
const bulkAction = async (action: string) => {
    ...
    if (action === 'delete') {
        ...
        confirmMessage = t('modules.system.users.messages.bulkForceLogoutConfirm', { count: selectedIds.value.length }) || `Force logout ${selectedIds.value.length} users?`;
        confirmTitle = t('modules.system.users.actions.forceLogout');
    } else if (action === 'verify') {
        confirmMessage = t('modules.system.users.messages.bulkVerifyConfirm', { count: selectedIds.value.length }) || `Verify ${selectedIds.value.length} users?`;
        confirmTitle = t('modules.system.users.actions.verify');
