...
const mediaSettingsGrouped = computed(() => {
    ...
    const groups: SettingGroupData[] = [
        {
            id: 'upload',
            title: t('modules.system.settings.groups.upload.title'),
            description: t('modules.system.settings.groups.upload.description'),
            icon: UploadIcon,
            color: 'blue',
    ...
    if (driver === 's3') {
        groups.push({
            id: 's3_config',
            title: t('modules.system.settings.groups.s3.title'),
            description: t('modules.system.settings.groups.s3.description'),
            icon: CloudIcon,
            color: 'orange',
    ...
    if (driver === 'google') {
        groups.push({
            id: 'google_config',
            title: t('modules.system.settings.groups.google.title'),
            description: t('modules.system.settings.groups.google.description'),
            icon: CloudIcon,
            color: 'red',
    ...
    if (driver === 'ftp') {
        groups.push({
            id: 'ftp_config',
            title: t('modules.system.settings.groups.ftp.title'),
            description: t('modules.system.settings.groups.ftp.description'),
            icon: ServerIcon,
            color: 'purple',
    ...
    if (driver === 'dropbox') {
        groups.push({
            id: 'dropbox_config',
            title: t('modules.system.settings.groups.dropbox.title'),
            description: t('modules.system.settings.groups.dropbox.description'),
            icon: CloudIcon,
            color: 'indigo',
    ...
    groups.push({
        id: 'image_processing',
        title: t('modules.system.settings.groups.imageProcessing.title'),
        description: t('modules.system.settings.groups.imageProcessing.description'),
        icon: ImageIcon,
        color: 'pink',
...
const testConnection = async () => {
    ...
    try {
        ...
        let msg = t('modules.system.settings.groups.test.failed') || 'Connection failed. Please check your credentials.';
...
const startMigration = async () => {
    const confirmed = await confirm({
        title: t('modules.system.settings.groups.migration.confirm_title'),
        message: t('modules.system.settings.groups.migration.confirm_message'),
        variant: 'warning',
        confirmText: t('modules.system.settings.groups.migration.start'),
    });
    ...
    try {
        ...
        if (totalFiles.value === 0) {
            ...
            migrationLogs.value.push({ type: 'info', message: t('modules.system.settings.groups.migration.no_files') || 'No files to migrate.' });
            return;
        }
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            if (stopMigration.value) {
                migrationLogs.value.push({ type: 'warning', message: t('modules.system.settings.groups.migration.stopped') || 'Migration stopped by user.' });
                break;
            }
        ...
        if (!stopMigration.value) {
            migrationStatus.value = 'completed';
            migrationLogs.value.push({ type: 'success', message: t('modules.system.settings.groups.migration.success') || 'Migration completed successfully.' });
        }
  ...
  <div class="space-y-4">
    ...
    >
      ...
      >
        <div class="flex items-center gap-4">
          ...
          >
            <span v-if="testingConnection">{{ $t('modules.system.settings.groups.test.testing') }}</span>
            <span v-else>{{ $t('modules.system.settings.groups.test.button') }}</span>
          </Button>
    ...
    >
      <h3 class="text-lg font-medium text-foreground mb-2">
        {{ $t('modules.system.settings.groups.migration.title') }}
      </h3>
      <p class="text-sm text-muted-foreground mb-4">
        {{ $t('modules.system.settings.groups.migration.description', { driver: formData.storage_driver }) }}
      </p>
      ...
      >
        <Button @click="startMigration">
          {{ $t('modules.system.settings.groups.migration.start') }}
        </Button>
      </div>
      ...
      >
        <div class="flex justify-between text-sm text-foreground">
          <span>
            <span v-if="migrationStatus === 'scanning'">{{ $t('modules.system.settings.groups.migration.scanning') }}</span>
            <span v-else>{{ $t('modules.system.settings.groups.migration.migrating', { processed: processedFiles, total: totalFiles }) }}</span>
          </span>
        ...
        >
          {{ $t('modules.system.settings.groups.migration.stop') }}
        </Button>
      </div>
