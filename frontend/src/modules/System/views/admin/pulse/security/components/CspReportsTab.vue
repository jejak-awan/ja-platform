    ...
    <CardHeader class="flex flex-row items-center justify-between pb-4">
      <div>
        <CardTitle class="text-lg">
          {{ $t('features.security.cspReports.title') }}
        </CardTitle>
    ...
    <div class="px-6 pb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="bg-muted/30 rounded-lg p-4">
        <p class="text-sm text-muted-foreground">
          {{ $t('features.security.cspReports.stats.total') }}
        </p>
      ...
      <div class="bg-muted/30 rounded-lg p-4">
        <p class="text-sm text-muted-foreground">
          {{ $t('features.security.cspReports.stats.new') }}
        </p>
      ...
      <div class="bg-muted/30 rounded-lg p-4">
        <p class="text-sm text-muted-foreground">
          {{ $t('features.security.cspReports.stats.topViolation') }}
        </p>
      ...
      <div class="bg-muted/30 rounded-lg p-4">
        <p class="text-sm text-muted-foreground">
          {{ $t('features.security.cspReports.stats.last24h') }}
        </p>
          ...
          <Select v-model="localFilters.status">
            ...
            <SelectContent>
              ...
              <SelectItem value="new">
                {{ $t('features.security.cspReports.status.new') }}
              </SelectItem>
              <SelectItem value="reviewed">
                {{ $t('features.security.cspReports.status.reviewed') }}
              </SelectItem>
              <SelectItem value="false_positive">
                {{ $t('features.security.cspReports.status.falsePositive') }}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
    ...
    >
      <div class="bg-muted/50 border border-border rounded-lg p-4 flex items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          >
            {{ $t('features.security.cspReports.actions.markReviewed') }}
          </Button>
          <Button
            variant="outline"
            size="sm"
          ...
          >
            {{ $t('features.security.cspReports.actions.markFalsePositive') }}
          </Button>
          <Button
            variant="destructive"
            size="sm"
...
const topViolation = computed(() => {
    ...
    return props.stats.by_directive[0]?.violated_directive || 'None';
});
