    ...
    <div class="mb-6 flex justify-between items-center">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-foreground">
          {{ t('modules.system.system.logs.title') }}
        </h1>
      </div>
      <div class="flex items-center space-x-2">
        <Button
          ...
          @click="clearLogs"
        >
          {{ clearing ? t('modules.system.system.logs.clearing') : t('modules.system.system.logs.clear') }}
        </Button>
      </div>
    </div>
        ...
        <div class="bg-card border border-border rounded-lg">
          <div class="px-6 py-4 border-b border-border">
            <h2 class="text-lg font-semibold text-foreground">
              {{ t('modules.system.system.logs.files') }}
            </h2>
          </div>
      ...
      <div class="lg:col-span-2">
        <div class="bg-card border border-border rounded-lg">
          <div class="px-6 py-4 border-b border-border flex items-center justify-between">
            <h2 class="text-lg font-semibold text-foreground">
              {{ selectedLogFile ? selectedLogFile.name : t('modules.system.system.logs.select') }}
            </h2>
            <div
            ...
            >
              <Input
                v-model="logSearch"
                type="text"
                :placeholder="t('modules.system.system.logs.search')"
                class="h-8 w-48"
              />
              <Button
                variant="outline"
                ...
                @click="refreshLog"
              >
                {{ t('modules.system.system.logs.refresh') }}
              </Button>
            </div>
          </div>
          <div class="p-6">
            ...
            >
              ...
              <p class="mt-4 text-muted-foreground">
                {{ t('modules.system.system.logs.empty') }}
              </p>
            </div>
            <div
            ...
            >
              <p class="text-muted-foreground">
                {{ t('modules.system.system.logs.loading') }}
              </p>
            </div>
            <div
              v-else
...
const selectLogFile = async (logFile: LogFile) : Promise<void> => {
    ...
    try {
        ...
        logContent.value = t('modules.system.system.logs.failed_load') || 'Failed to load log content';
    } finally {
