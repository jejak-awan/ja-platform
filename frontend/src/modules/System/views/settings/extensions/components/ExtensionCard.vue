    ...
    <CardContent class="p-6 flex-1">
      ...
      <div class="flex items-start justify-between gap-4">
        ...
        <div class="flex flex-col items-end gap-1.5 shrink-0">
          <Badge :variant="ext.status === 'active' ? 'success' : 'secondary'">
            {{ ext.status === 'active' ? (locale === 'id' ? 'Aktif' : 'Active') : (locale === 'id' ? 'Nonaktif' : 'Inactive') }}
          </Badge>
          
          <!-- Elegant Dynamic License Tier Badge -->
          <span 
          ...
          <span 
            ...
            class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-500/15 text-indigo-400 border border-indigo-500/30"
          >
            PRO License
          </span>
          <span 
      ...
      <div class="mt-6 space-y-1.5 border-t border-border/30 pt-4 text-xs">
        <div class="flex justify-between">
          <span class="text-muted-foreground">{{ trans.author }}:</span>
          <span class="font-medium text-foreground">{{ ext.author || 'jejakawan' }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-muted-foreground">{{ trans.license }}:</span>
          <span class="font-mono text-foreground">
            {{ getExtensionLicenseTier(ext.slug) === 'free' ? 'MIT' : (getExtensionLicenseTier(ext.slug) === 'pro' ? 'Commercial PRO' : 'Enterprise Proprietary') }}
          </span>
        </div>
        <div class="flex justify-between" v-if="ext.is_core">
          <span class="text-muted-foreground">{{ trans.core }}:</span>
          <span class="text-indigo-400 font-semibold">{{ trans.yes }}</span>
        </div>
      </div>
      ...
      <div v-if="ext.features && ext.features.length > 0" class="mt-4 border-t border-border/30 pt-3">
        ...
        >
          <span class="flex items-center gap-2 font-semibold text-foreground">
            ...
            {{ trans.constituentFeatures }} ({{ ext.features.length }})
          </span>
          <component
        ...
        >
          ...
          <div v-if="groupedFeatures.core.length > 0" class="space-y-1.5">
            <div class="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-muted-foreground/80 px-1 border-b border-border/20 pb-1">
              <span>Core Base Infrastructure</span>
              <span class="text-indigo-400 font-mono">Mandatory Service</span>
            </div>
            ...
            >
              <div class="flex-1 min-w-0 pr-2">
                ...
                <p class="text-[10px] text-muted-foreground mt-0.5 line-clamp-1" :title="feat.description">
                  {{ feat.description || 'Mandatory core system infrastructure.' }}
                </p>
              </div>
          ...
          <div v-if="groupedFeatures.operational.length > 0" class="space-y-1.5">
            <div class="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-muted-foreground/80 px-1 border-b border-border/20 pb-1">
              <span>Operational Extensions</span>
              <span class="text-emerald-500">Active Toggleable</span>
            </div>
