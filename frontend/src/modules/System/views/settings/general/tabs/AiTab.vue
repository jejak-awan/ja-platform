<template>
  <div class="space-y-6">
    <SettingGroup
      :title="$t('modules.system.settings.ai.title', 'AI Assistance')"
      :description="$t('modules.system.settings.ai.description', 'Configure AI features for content generation.')"
      :icon="Sparkles"
      color="indigo"
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <SettingField
          ...
          type="boolean"
          :label="$t('modules.system.settings.ai.enable_ai', 'Enable AI Features')"
          :description="$t('modules.system.settings.ai.enable_ai_desc', 'Enable or disable AI assistance in the editor.')"
        ...
        <div>
          <label class="block text-sm font-medium text-foreground mb-1">
            {{ $t('modules.system.settings.ai.default_provider', 'Default Provider') }}
          </label>
          <p class="text-xs text-muted-foreground mb-2">
            {{ $t('modules.system.settings.ai.default_provider_desc', 'Select which AI provider to use by default.') }}
          </p>
          <Select 
          ...
          >
            <SelectTrigger>
              <SelectValue :placeholder="$t('modules.system.settings.ai.default_provider')" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem
    ...
    >
      <h3 class="text-lg font-medium">
        {{ $t('modules.system.settings.ai.providers', 'AI Providers') }}
      </h3>
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              >
                {{ $t('modules.system.settings.groups.ai.using_model', { model: formData[`${provider.id}_model`] }) }}
              </span>
            </div>
          </div>
          <ChevronDown 
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1">
              <label class="block text-sm font-medium text-foreground">
                {{ $t('modules.system.settings.ai.api_key', 'API Key') }}
              </label>
              <div class="relative">
                <Input 
                  :type="showKey[provider.id] ? 'text' : 'password'"
                  :placeholder="$t('modules.system.settings.ai.api_key_placeholder', 'Enter API Key')"
            ...
            <div class="space-y-1">
              <label class="block text-sm font-medium text-foreground">
                {{ $t('modules.system.settings.ai.model_select', 'Default Model') }}
              </label>
              <div class="flex gap-2">
                ...
                >
                  <SelectTrigger class="w-full">
                    <SelectValue :placeholder="loadingModels[provider.id] ? $t('modules.system.settings.groups.ai.loading_models') : ((formData[`${provider.id}_model`] as string) || $t('modules.system.settings.groups.ai.select_model'))" />
                  </SelectTrigger>
                ...
                <Button 
                  ...
                  variant="outline" 
                  type="button" 
                  :title="$t('modules.system.settings.ai.test_connection')"
                ...
                <Button 
                  ...
                  variant="ghost" 
                  type="button" 
                  :title="$t('modules.system.settings.ai.fetch_models')"
              ...
              >
                {{ $t('modules.system.settings.groups.ai.enter_api_key_to_select') }}
              </p>
            </div>
          </div>
        </div>
...
const fetchModels = async (providerId: string, force = false) => {
    ...
    try {
        ...
        if (force) {
            toast.success.action(t('modules.system.settings.groups.ai.fetch_success') || 'Models fetched successfully');
        }
    } catch {
...
const testConnection = async (providerId: string) => {
    ...
    try {
        ...
        toast.success.action(t('modules.system.settings.groups.ai.connection_success') || 'Connection successful');
