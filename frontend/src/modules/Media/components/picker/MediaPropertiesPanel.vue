    ...
    >
      ...
      <div class="p-3 border-b border-border/40 flex items-center justify-between bg-transparent shrink-0">
        <h3 class="font-semibold text-xs uppercase tracking-widest text-muted-foreground/80">
          {{ $t('modules.system.media.modals.edit.title') }}
        </h3>
        <Button
          variant="ghost"
          size="icon"
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-3 p-3 bg-muted/30 rounded-lg border border-border/20 text-xs">
          <div>
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.system.media.modals.edit.size') }}</span>
            <span class="font-medium text-foreground">{{ formatFileSize(activeMedia.size) }}</span>
          </div>
          <div class="truncate">
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.system.media.modals.edit.type') }}</span>
            <span
          ...
          <div class="col-span-2 truncate">
            <span class="text-muted-foreground block mb-0.5">{{ $t('modules.system.media.modals.edit.folder') }}</span>
        ...
        >
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.system.media.modals.edit.name') }}
            </label>
            <Input
              v-model="form.name"
              type="text"
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.system.media.modals.edit.altText') }}
            </label>
            <Input
              v-model="form.alt"
              type="text"
              :placeholder="$t('modules.system.media.modals.edit.altPlaceholder')"
              class="bg-background h-8 text-xs"
            />
          </div>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.system.media.modals.edit.caption') || 'Caption' }}
            </label>
            <Input
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.system.media.modals.edit.description') }}
            </label>
            <textarea
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-foreground">
              {{ $t('modules.system.media.modals.edit.tags') || 'Tags' }}
            </label>
          ...
          <div class="pt-3 sticky bottom-0 pb-2 mt-4 bg-transparent">
            <div class="flex gap-2">
              ...
              >
                {{ saving ? $t('modules.system.media.modals.edit.saving') : $t('modules.system.media.actions.save') }}
              </Button>
              <Button 
                type="button"
                variant="outline"
                ...
                :title="$t('modules.system.media.actions.copyUrl')"
                @click="copyUrl"
              >
                <Link class="w-4 h-4" />
              </Button>
              <Button 
                ...
                size="icon"
                class="shrink-0 h-9 w-9"
                :title="$t('modules.system.media.actions.download')"
      ...
      >
        ...
        <p class="text-sm font-medium">
          {{ $t('modules.system.media.modals.edit.select_item') || 'Select item to view details' }}
        </p>
      </div>
    </div>
  </div>
...
const copyUrl = async () => {
    if (activeMedia.value?.url) {
        try {
            ...
            toast.success.action(t('modules.system.media.toast.urlCopied'));
        } catch (_err) {
...
const handleSubmit = async () => {
    ...
    try {
        ...
        toast.success.action(t('modules.system.media.messages.updateSuccess'));
    } catch (error) {
