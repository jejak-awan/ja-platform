...
import { contentSchema } from '@/modules/Cms/schemas/content';
...
useHead({
    title: computed(() => `${t('modules.cms.content.list.createNew')} | ${systemStore.siteSettings?.site_name || 'JA CMS'}`)
});
...
const handleSubmit = async (status: string | null = null) => {
    ...
    try {
        ...
        const endpoint = contentId.value
            ? `/manage/cms/contents/${contentId.value}`
            : '/manage/cms/contents';
...
const resolveConflict = async (action: 'unique' | 'force_delete') => {
    ...
    if (action === 'unique' && suggested) {
        ...
        try {
            loading.value = true;
            await api.delete(`/manage/cms/contents/${conflictId}/force-delete`);
            slugConflict.value = null;
