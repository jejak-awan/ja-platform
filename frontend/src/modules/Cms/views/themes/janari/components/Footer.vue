...
const siteVersion = computed(() => siteSettings.value?.site_version || 'Theme Janari v1.0');
...
const footerCol1Items = computed(() => {
    ...
    return (menu?.items && menu.items.length > 0) ? menu.items : defaultCol1Items;
});
const footerCol2Items = computed(() => {
    const menu = menus.value['footer_col_2'];
    return (menu?.items && menu.items.length > 0) ? menu.items : defaultCol2Items;
});
