<template>
  <div class="min-h-screen flex flex-col">
    ...
    >
      <PageDisabled 
        :title="(pageTitle as string) || 'Berita & Informasi'" 
    ...
    <div v-else class="flex-1 flex flex-col bg-background">
        ...
        <section class="py-20 bg-gradient-to-b from-primary/10 to-background dark:from-primary/20">
          <div class="container mx-auto px-4 text-center">
            <span class="text-primary font-bold tracking-wider uppercase text-xs mb-4 block">{{ pageTitle || 'Portal Berita & Informasi' }}</span>
            <h1
              ref="blogTitle"
              class="text-4xl md:text-5xl font-extrabold mb-4 text-foreground"
            >
              <JanariSplitText :text="pageTitle || 'Kabar Skansaci'" />
            </h1>
            <p class="text-muted-foreground max-w-lg mx-auto font-medium">
              {{ pageSubtitle || 'Informasi terkini mengenai kegiatan sekolah, prestasi siswa, dan pengumuman resmi.' }}
            </p>
          </div>
        </section>
              ...
              <div class="lg:col-span-9 articles-scroll-anchor-off">
                <div class="flex items-center justify-between mb-6">
                  <h2 class="text-sm font-bold text-muted-foreground uppercase tracking-wider">
                    {{ isSearchMode ? 'Search Results' : 'Latest Articles' }}
                  </h2>
                  ...
                  <div v-if="searching || loading" class="flex items-center gap-2 text-primary">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-widest">{{ searching ? 'Searching...' : 'Loading...' }}</span>
                  </div>
                </div>
                ...
                >
                  <div v-if="loadingMore" class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-xs text-muted-foreground animate-pulse">Memuat lebih banyak...</span>
                  </div>
                </div>
                ...
                >
                  <p class="text-muted-foreground italic">Tidak ada konten yang cocok dengan pencarian/kategori saat ini.</p>
                  <div
...
const estimateReadTime = (text: string | undefined): string => {
    ...
    return `${Math.max(1, Math.ceil(text.split(' ').length / 200))} min`
}
