<template>
  <div class="min-h-screen bg-background transition-colors duration-300 overflow-x-hidden">
    ...
    >
      <PageDisabled 
        :title="(pageTitle as string) || 'Hubungi Kami'" 
      ...
      >
        <section class="relative overflow-hidden border-y border-border/50 aspect-[16/9] md:aspect-[21/9] lg:aspect-[3/1] min-h-[300px]">
          ...
          <div class="container mx-auto px-6 py-16 md:py-20 relative z-10">
            <span class="text-primary font-bold tracking-wider uppercase text-sm mb-4 block">{{ pageTitle || 'Kontak Kami' }}</span>
...
const pageSubtitle = computed(() =>
    ...
    'Punya pertanyaan seputar PPDB, Jurusan, atau kerjasama industri? Kami siap membantu menjawab kebutuhan Anda.'
)
...
async function copyToClipboard(value: string, okMessage: string): Promise<void> {
    ...
    try {
        ...
        toast.error.default('Gagal menyalin data.')
    }
}
...
async function copyLocationAddress(): Promise<void> {
    await copyToClipboard(String(displayAddress.value || ''), 'Alamat berhasil disalin.')
}
...
async function copyLocationCoordinates(): Promise<void> {
    ...
    if (!coords) {
        toast.error.default('Koordinat belum tersedia di link map saat ini.')
        return
    }
    await copyToClipboard(coords, 'Koordinat berhasil disalin.')
}

async function shareLocation(): Promise<void> {
    const title = String(pageTitle.value || 'Lokasi')
    const text = String(displayAddress.value || 'Lokasi sekolah')
    const url = mapExternalUrl.value
    ...
    await copyToClipboard(`${text}\n${url}`, 'Link lokasi berhasil disalin.')
}
...
async function loadContactForm(): Promise<void> {
    ...
    try {
        ...
        if (status === 404) {
            ...
            formLoadError.value = 'Tidak dapat memuat formulir. Silakan refresh halaman atau coba lagi nanti.'
        }
...
async function submitForm(): Promise<void> {
    ...
    try {
        ...
        if (axios.isAxiosError(e) && e.response?.status === 422) {
            ...
            toast.error.default(msg || 'Pengiriman gagal. Coba lagi.')
        } else {
            toast.error.default('Pengiriman gagal. Coba lagi.')
        }
        await captchaRef.value?.refresh?.()
        captchaPayload.value = null
    } finally {
