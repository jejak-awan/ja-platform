  ...
  <div class="min-h-screen bg-background">
      ...
      >
        <div class="mx-auto max-w-2xl rounded-2xl border border-dashed border-border bg-card/40 p-10 text-center">
          ...
          <p class="mt-4 text-muted-foreground">
            Halaman ini sudah terpublikasi, tapi konten utamanya belum diisi.
          </p>
        </div>
      </section>
    </template>
    ...
    >
      <div class="mx-auto max-w-2xl rounded-2xl border border-border bg-card/40 p-10 text-center">
        ...
        <p class="mt-4 text-muted-foreground">
          Halaman yang kamu tuju tidak tersedia atau belum dipublikasikan.
        </p>
      </div>
    </section>
  </div>
...
const pageTitle = computed(() => pageData.value?.title || pageSlug.value.replace(/-/g, ' ') || 'Page');
const pageIntro = computed(() => pageData.value?.intro?.trim() || '');
const hasBody = computed(() => Boolean(pageData.value?.body && pageData.value.body.trim()));
