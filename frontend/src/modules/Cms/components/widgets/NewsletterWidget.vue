<template>
  ...
  >
    <div class="newsletter-content">
      ...
      <h3
        v-if="title || t('features.frontend.newsletter.title')"
        class="newsletter-title"
      >
        {{ title || t('features.frontend.newsletter.title') }}
      </h3>
      <p
        v-if="description || t('features.frontend.newsletter.description')"
      ...
      >
        ...
        <a
          href="/privacy"
          class="privacy-link"
        >{{ t('features.frontend.newsletter.privacyLink') }}</a>.
      </p>
    </div>
  </div>
</template>
...
const handleSubscribe = async () => {
  ...
  try {
    const response = await api.post('/public/cms/newsletter/subscribe', {
      email: email.value,
    });
