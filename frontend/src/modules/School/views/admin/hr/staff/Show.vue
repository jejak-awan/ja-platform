  ...
  <div class="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div class="space-y-1">
        <div class="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          <router-link :to="{ name: 'staff.index' }" class="hover:text-primary transition-colors">{{ $t('features.school.hr.staff.title') }}</router-link>
          <LucideIcon name="ChevronRight" class="w-3 h-3" />
          <span class="text-foreground font-medium">{{ staff.full_name }}</span>
        </div>
        <h1 class="text-3xl font-extrabold tracking-tight">{{ staff.full_name }}</h1>
      </div>
      ...
      <div class="flex gap-2">
        ...
        <Button
          variant="secondary"
          size="lg"
          class="bg-muted/50 hover:bg-muted/80 transition-colors"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-1 space-y-6">
        <Card class="overflow-hidden border-border/50 bg-background/50 backdrop-blur-sm shadow-xl rounded-2xl sticky top-24">
          <div class="h-32 bg-gradient-to-br from-primary/20 via-accent/10 to-background border-b border-border/50 relative">
             <div class="absolute -bottom-12 left-1/2 -translate-x-1/2">
                <div class="w-24 h-24 rounded-2xl bg-background p-1 shadow-2xl border border-border/50">
      ...
      <div class="lg:col-span-2 space-y-6">
        <Tabs v-model="activeTab" class="w-full">
          <TabsList class="bg-muted/30 p-1 rounded-xl border border-border/50 w-full justify-start overflow-x-auto h-auto">
            <TabsTrigger value="profile" class="rounded-lg px-6 py-3 transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <LucideIcon name="User" class="w-4 h-4 mr-2" /> {{ $t('features.school.hr.staff.tabs.profile') || 'Profil' }}
            </TabsTrigger>
            <TabsTrigger value="employment" class="rounded-lg px-6 py-3 transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <LucideIcon name="History" class="w-4 h-4 mr-2" /> {{ $t('features.school.hr.staff.tabs.employment') || 'Kepegawaian' }}
            </TabsTrigger>
            <TabsTrigger value="education" class="rounded-lg px-6 py-3 transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm">
          ...
          <div class="mt-6">
            ...
            <TabsContent value="profile" class="space-y-6 animate-in slide-in-from-right-2 duration-300">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Personal Info Card -->
                <Card class="border-border/50 bg-background/50 shadow-lg rounded-2xl overflow-hidden">
                  <CardHeader class="border-b border-border/30 bg-muted/20">
                ...
                <Card class="border-border/50 bg-background/50 shadow-lg rounded-2xl overflow-hidden">
                  ...
                  <CardContent class="p-6 space-y-4">
                    ...
                    <div class="space-y-1.5 pt-2">
                       <span class="text-xs text-muted-foreground uppercase tracking-widest font-bold">Alamat Rumah</span>
               ...
               <Card class="border-border/50 bg-background/50 shadow-lg rounded-2xl overflow-hidden">
                  ...
                  <CardContent class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                      ...
                      <DetailItem label="Status Sertifikasi" :value="staff.certification_status ? 'Sudah Sertifikasi' : 'Belum Sertifikasi'" icon="CheckCircle2" />
                    </div>
                  </CardContent>
               </Card>
            </TabsContent>
               ...
               <Card class="border-border/50 bg-background/50 shadow-lg rounded-2xl overflow-hidden">
                  ...
                  <CardContent class="p-6 space-y-6">
                    <div class="flex items-start gap-4">
                       ...
                       <div class="space-y-1">
                          <h4 class="font-bold text-lg leading-none">{{ staff.last_education || 'Tidak Terdata' }}</h4>
...
const DetailItem = (props: { label: string, value: any, icon: string }) => {
  return h('div', { class: 'space-y-1.5' }, [
    h('span', { class: 'text-xs text-muted-foreground uppercase tracking-widest font-bold' }, props.label),
    h('div', { class: 'flex items-center gap-2 group' }, [
       ...
       h('span', { class: 'font-medium text-foreground truncate' }, props.value || '-')
    ])
  ]);
};
