  ...
  <Card class="border border-border/40 shadow-none rounded-xl">
    <CardHeader class="border-b border-border/40 bg-muted/20 pb-4">
      <div class="flex justify-between items-center">
        <div>
          <CardTitle class="text-lg">Pengaturan Kelulusan</CardTitle>
          <CardDescription>Atur status pengumuman, jadwal, dan mapping mata pelajaran</CardDescription>
        </div>
    ...
    <CardContent class="p-6">
      <div class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase text-muted-foreground">Pilih Tahun Ajaran</label>
          ...
          <div class="flex items-end">
            <Button v-if="!currentSetting" variant="outline" class="w-full rounded-xl border-dashed" @click="createSetting">
              ...
              Buat Pengaturan Tahun {{ selectedYear }}
            </Button>
          </div>
        </div>
        ...
        <div v-if="currentSetting" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center space-x-2">
                ...
                <label for="is-open" class="text-sm font-medium leading-none cursor-pointer">
                  Buka Pengumuman Kelulusan
                </label>
              </div>
              <p class="text-xs text-muted-foreground ml-11">
                Jika diaktifkan, siswa dapat melihat hasil kelulusan di halaman publik.
              </p>

              <div class="space-y-2 pt-2">
                <label class="text-xs font-bold uppercase text-muted-foreground">Jadwal Pengumuman</label>
                <Input v-model="form.announcement_date" type="datetime-local" class="rounded-xl w-full" />
                <p class="text-xs text-muted-foreground">Kosongkan jika pengumuman dapat diakses kapan saja setelah dibuka.</p>
              </div>
            </div>
            ...
            <div class="space-y-4">
              <h3 class="text-sm font-bold flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  Mata Pelajaran (Mapping)
                </div>
                <Button variant="ghost" size="sm" class="h-8 px-2 text-xs" @click="addSubject">
                  <LucideIcon name="Plus" class="w-3 h-3 mr-1" /> Tambah
                </Button>
              </h3>
              
              <p class="text-xs text-muted-foreground mb-2">
                Daftar mata pelajaran ini akan digunakan sebagai format saat Import Excel.
              </p>

              <div class="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                <div v-for="(_, idx) in form.subjects" :key="idx" class="flex items-center gap-2">
                  <Input v-model="form.subjects[idx]" placeholder="Nama Mata Pelajaran" class="flex-1 rounded-lg text-sm" />
                ...
                <div v-if="!form.subjects || form.subjects.length === 0" class="text-center py-4 text-xs italic text-muted-foreground border-2 border-dashed border-border rounded-xl">
                  Belum ada mata pelajaran. Klik tambah.
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-border/40">
            <Button variant="outline" class="rounded-xl px-6" @click="fetchSettings" :disabled="saving">Batal</Button>
...
const createSetting = async () => {
    ...
    try {
        ...
        toast.success.action('Pengaturan baru berhasil dibuat');
        await fetchSettings();
    } catch (e) {
        toast.error.fromResponse(e);
    } finally {
...
const saveSettings = async () => {
    ...
    try {
        ...
        toast.success.action('Pengaturan berhasil disimpan');
        await fetchSettings();
    } catch (e) {
        toast.error.fromResponse(e);
    } finally {
