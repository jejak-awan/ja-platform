  ...
  <Card>
    <CardHeader class="flex flex-row items-center justify-between pb-4">
      <div>
        <CardTitle class="text-lg">
          {{ showAllPackages ? $t('features.security.packages.title') : $t('features.security.vulnerabilities.title') }}
        </CardTitle>
    ...
    <div class="px-6 pb-4 grid grid-cols-1 md:grid-cols-5 gap-4">
      <template v-if="showAllPackages">
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.packages.stats.total') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.packages.stats.composer') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.packages.stats.npm') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.packages.stats.secure') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.packages.stats.vulnerable') }}
          </p>
      ...
      <template v-else>
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.vulnerabilities.stats.total') }}
          </p>
          <p class="text-2xl font-bold text-foreground">
            {{ stats.total || 0 }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.vulnerabilities.stats.critical') }}
          </p>
          <p class="text-2xl font-bold text-red-600">
            {{ stats.critical || 0 }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.vulnerabilities.stats.high') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.vulnerabilities.stats.medium') }}
          </p>
        ...
        <div class="bg-muted/30 rounded-lg p-4">
          <p class="text-sm text-muted-foreground">
            {{ $t('features.security.vulnerabilities.stats.low') }}
          </p>
          <p class="text-2xl font-bold text-blue-600">
            {{ stats.low || 0 }}
          </p>
          ...
          <Select v-model="localFilters.source">
            ...
            <SelectContent>
              ...
              <SelectItem value="composer">
                {{ $t('features.security.vulnerabilities.source.composer') }}
              </SelectItem>
              <SelectItem value="npm">
                {{ $t('features.security.vulnerabilities.source.npm') }}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
            ...
            <Select v-model="localFilters.severity">
              ...
              <SelectContent>
                ...
                <SelectItem value="critical">
                  {{ $t('features.security.vulnerabilities.severity.critical') }}
                </SelectItem>
                <SelectItem value="high">
                  {{ $t('features.security.vulnerabilities.severity.high') }}
                </SelectItem>
                <SelectItem value="medium">
                  {{ $t('features.security.vulnerabilities.severity.medium') }}
                </SelectItem>
                <SelectItem value="low">
                  {{ $t('features.security.vulnerabilities.severity.low') }}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
            ...
            <Select v-model="localFilters.status">
              ...
              <SelectContent>
                ...
                <SelectItem value="new">
                  {{ $t('features.security.vulnerabilities.status.new') }}
                </SelectItem>
                <SelectItem value="acknowledged">
                  {{ $t('features.security.vulnerabilities.status.acknowledged') }}
                </SelectItem>
                <SelectItem value="patched">
                  {{ $t('features.security.vulnerabilities.status.patched') }}
                </SelectItem>
                <SelectItem value="ignored">
                  {{ $t('features.security.vulnerabilities.status.ignored') }}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <Select v-model="packageFilters.status">
            ...
            <SelectContent>
              ...
              <SelectItem value="secure">
                {{ $t('features.security.packages.status.secure') }}
              </SelectItem>
              <SelectItem value="vulnerable">
                {{ $t('features.security.packages.status.vulnerable') }}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
...
const fetchPackages = async (): Promise<void> => {
    ...
    try {
        ...
        const response = await api.get('/admin/core/security/dependency-packages', { params });
...
const fetchPackageStats = async (): Promise<void> => {
    try {
        const response = await api.get('/admin/core/security/dependency-packages/statistics');
