...
const fetchPlugins = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/core/plugins');
...
const togglePlugin = async (plugin: Plugin) => {
    try {
        if (plugin.is_active) {
            await api.post(`/admin/core/plugins/${plugin.id}/deactivate`);
            plugin.is_active = false;
            toast.success.action(t('features.developer.plugins.messages.deactivated'));
        } else {
            await api.post(`/admin/core/plugins/${plugin.id}/activate`);
