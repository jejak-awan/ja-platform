...
const fetchWebhooks = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/core/webhooks');
        const { data } = parseResponse(response);
        webhooks.value = ensureArray(data);
        
        // Fetch statistics
        try {
            const statsResponse = await api.get('/admin/core/webhooks/statistics');
...
const testWebhook = async (webhook: Webhook) => {
    try {
        await api.post(`/admin/core/webhooks/${webhook.id}/test`);
...
const deleteWebhook = async (webhook: Webhook) => {
    ...
    try {
        await api.delete(`/admin/core/webhooks/${webhook.id}`);
