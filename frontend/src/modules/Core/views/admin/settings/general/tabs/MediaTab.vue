...
const testConnection = async () => {
    ...
    try {
        ...
        const response = await api.post('/admin/cms/settings/test-storage', {
...
const startMigration = async () => {
    ...
    try {
        // 1. Scan files
        const response = await api.get('/admin/core/storage/migration/files');
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            ...
            try {
                const res = await api.post('/admin/core/storage/migration/batch', { files: batch });
