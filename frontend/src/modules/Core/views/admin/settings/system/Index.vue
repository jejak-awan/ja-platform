      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-muted-foreground">
                {{ t('features.system.info.health.title') }}
              </p>
              <p
        ...
        <div class="bg-card border border-border rounded-lg p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm font-medium text-muted-foreground">
                {{ t('features.system.info.cache.title') }}
              </p>
              <p class="text-2xl font-semibold text-foreground">
                {{ cacheStatus || t('features.system.info.cache.active') }}
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="grid grid-cols-1 gap-6">
            <div>
              <h3 class="text-sm font-medium text-foreground mb-3 font-bold border-b pb-1">
                {{ t('features.system.info.sections.application') }}
              </h3>
              <dl class="space-y-2">
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.phpVersion') }}
                  </dt>
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.laravelVersion') }}
                  </dt>
                  <dd class="text-sm text-foreground font-mono">
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.environment') }}
                  </dt>
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.debugMode') }}
                  </dt>
                  <dd
                    class="text-sm"
                    :class="systemInfo.debug_mode ? 'text-red-500' : 'text-green-500'"
                  >
                    {{ systemInfo.debug_mode ? t('features.system.info.sections.enabled') : t('features.system.info.sections.disabled') }}
                  </dd>
                </div>
              </dl>
            </div>
            <div>
              <h3 class="text-sm font-medium text-foreground mb-3 font-bold border-b pb-1">
                {{ t('features.system.info.sections.server') }}
              </h3>
              <dl class="space-y-2">
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.serverSoftware') }}
                  </dt>
                  <dd
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.memoryUsage') }}
                  </dt>
                  <dd class="text-sm text-foreground font-mono">
                    {{ displayMemory }}
                  </dd>
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.diskUsage') }}
                  </dt>
                  <dd class="text-sm text-foreground font-mono">
                    {{ displayDisk }}
                  </dd>
                ...
                <div class="flex justify-between">
                  <dt class="text-sm text-muted-foreground">
                    {{ t('features.system.info.sections.database') }}
                  </dt>
        ...
        <div class="bg-card border border-border rounded-lg p-6">
          <h2 class="text-lg font-semibold text-foreground mb-4">
            {{ t('features.system.info.quickActions.title') }}
          </h2>
...
const fetchSystemInfo = async (): Promise<void> => {
    loading.value = true;
    try {
        const response = await api.get('/admin/core/system/info');
        systemInfo.value = parseSingleResponse<SystemInfo>(response) || {};

        // Fetch cache status
        try {
            const cacheResponse = await api.get('/admin/core/system/cache-status');
