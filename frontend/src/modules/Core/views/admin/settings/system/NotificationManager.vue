  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-3xl font-bold tracking-tight text-foreground">
        {{ $t('features.system.notifications.title') }}
      </h1>
    </div>
        ...
        <Card>
          ...
          <CardContent class="space-y-4 pt-2">
              ...
              <Select v-model="form.target_type">
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {{ $t('features.system.notifications.form.targets.all') }}
                  </SelectItem>
                  <SelectItem value="role">
                    {{ $t('features.system.notifications.form.targets.role') }}
                  </SelectItem>
                  <SelectItem value="user">
                    {{ $t('features.system.notifications.form.targets.user') }}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            >
              ...
              <div>
                <p class="font-semibold">
                  {{ $t('features.system.notifications.queue.inactive_title') }}
                </p>
            ...
            <div class="space-y-2">
              ...
              <Input
                v-model="form.title"
                :placeholder="$t('features.system.notifications.form.title')"
              />
            </div>
...
const fetchQueueHealth = async (): Promise<void> => {
    try {
        const res = await api.get('/admin/core/system/info');
...
const fetchData = async (): Promise<void> => {
    try {
        const [rolesRes, usersRes] = await Promise.all([
            api.get('/admin/core/roles'),
            api.get('/admin/core/users')
        ]);
...
const fetchHistory = async (page = 1) : Promise<void> => {
    try {
        const res = await api.get('/admin/core/notifications/system', {
...
const handleBulkRevoke = async () => {
    ...
    try {
        await api.post('/admin/core/notifications/system/bulk-revoke', {
...
const handleRevoke = async (notification: Notification) => {
    ...
    try {
        await api.post('/admin/core/notifications/system/revoke', {
...
const handleSend = async () => {
    ...
    sending.value = true;
    try {
        await api.post('/admin/core/notifications/broadcast', form);
