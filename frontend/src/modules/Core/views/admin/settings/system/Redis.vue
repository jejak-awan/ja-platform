...
const loadSettings = async () : Promise<void> => {
  try {
    const response = await api.get('/admin/core/redis/settings')
...
const saveSettings = async () : Promise<void> => {
  ...
  try {
    ...
    await api.put('/admin/core/redis/settings', {
...
const loadStats = async () : Promise<void> => {
  ...
  try {
    const response = await api.get('/admin/core/redis/info')
...
const flushCache = async (type: string) : Promise<void> => {
  ...
  flushing.value = true
  try {
    await api.post('/admin/core/redis/flush-cache', { type })
...
const warmCache = async () : Promise<void> => {
  ...
  warming.value = true
  try {
    await api.post('/admin/core/redis/warm-cache')
