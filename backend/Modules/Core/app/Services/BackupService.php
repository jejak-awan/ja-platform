...
class BackupService
{
    ...
    public function getScheduleSettings(): array
    {
        ...
        $enabledRaw = \Modules\Core\Models\Setting::get('backup_schedule_enabled', false);
        $enabled = is_bool($enabledRaw) ? $enabledRaw : (bool) $enabledRaw;

        /** @var mixed $frequencyRaw */
        $frequencyRaw = \Modules\Core\Models\Setting::get('backup_schedule_frequency', 'daily');
        $frequency = is_string($frequencyRaw) ? $frequencyRaw : 'daily';

        /** @var mixed $timeRaw */
        $timeRaw = \Modules\Core\Models\Setting::get('backup_schedule_time', '02:00');
        $time = is_string($timeRaw) ? $timeRaw : '02:00';

        /** @var mixed $retentionDaysRaw */
        $retentionDaysRaw = \Modules\Core\Models\Setting::get('backup_retention_days', 30);
        $retentionDays = is_numeric($retentionDaysRaw) ? (int) $retentionDaysRaw : 30;

        /** @var mixed $maxBackupsRaw */
        $maxBackupsRaw = \Modules\Core\Models\Setting::get('backup_max_count', 10);
        $maxBackups = is_numeric($maxBackupsRaw) ? (int) $maxBackupsRaw : 10;

        /** @var mixed $lastRunRaw */
        $lastRunRaw = \Modules\Core\Models\Setting::get('backup_last_run');
    ...
    public function updateScheduleSettings(array $settings): array
    {
        ...
        foreach ($settings as $key => $value) {
            if (isset($typeMap[$key])) {
                \Modules\Core\Models\Setting::set($key, $value, $typeMap[$key], 'backup');
            }
        }
    ...
    public function runScheduledBackup(): ?Backup
    {
        ...
        \Modules\Core\Models\Setting::set('backup_last_run', now()->toISOString(), 'string', 'backup');
