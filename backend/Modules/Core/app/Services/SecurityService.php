...
class SecurityService
{
    ...
    {
        $this->maxFailedAttempts = is_scalar($v1 = \Modules\Core\Models\Setting::get('login_attempts_limit', 5)) ? (int) $v1 : 5;
        $this->baseBlockMinutes = is_scalar($v2 = \Modules\Core\Models\Setting::get('block_duration_minutes', 15)) ? (int) $v2 : 15;
    ...
    public function isShieldVerified(string $ipAddress, string $userAgent): bool
    {
        ...
        $ipKey = 'shield:verified:'.md5($ipAddress.'|'.$userAgent);
    ...
    protected function generateTrustToken(string $ipAddress, string $userAgent): string
    {
        ...
        return md5($subnet.'|'.$userAgent.'|'.$appKeyString);
    }
    ...
    public function recordShieldVerification(string $ipAddress, string $userAgent, ?string $fingerprint = null): void
    {
        ...
        if (\Modules\Core\Models\Setting::get('shield_log_verification_success', false)) {
    ...
    {
        $raw = \Modules\Core\Models\Setting::get('shield_trust_ttl_minutes', 10080);
    ...
    {
        $settingValue = \Modules\Core\Models\Setting::get('shield_protection_difficulty', 4);
        ...
        if ($ip) {
            $anomalyService = app(\Modules\Core\Services\AnomalyDetectionService::class);
