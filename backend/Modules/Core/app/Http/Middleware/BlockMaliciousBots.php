...
class BlockMaliciousBots
{
    ...
    protected \Modules\Core\Services\SecurityService $securityService;

    protected \Modules\Core\Services\AnomalyDetectionService $anomalyService;

    public function __construct(
        \Modules\Core\Services\SecurityService $securityService,
        \Modules\Core\Services\AnomalyDetectionService $anomalyService
    ) {
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (app(\Modules\Core\Services\SecurityMaintenanceService::class)->isModulePaused('scanner') || $this->shouldBypassMaintenance($request)) {
            return $next($request);
        }
        ...
        $learnedPaths = \Modules\Core\Models\Setting::get('security_learned_scanner_paths', []);
        ...
        foreach ($blockedPaths as $blockedPath) {
            if (str_contains($path, $blockedPath)) {
                Log::channel('security')->warning('Blocked malicious scanner', [
                    'ip' => $ip,
                    'path' => $path,
