...
class SecurityController extends BaseApiController
{
    ...
    protected \Modules\Core\Services\AttackCorrelationService $correlationService;

    protected \Modules\Core\Services\FileIntegrityService $fileIntegrityService;

    protected \Modules\Core\Services\SecurityNotificationService $notificationService;

    protected \Modules\Core\Services\SecurityAssessmentService $assessmentService;

    public function __construct(
        SecurityService $securityService,
        SecurityAlertService $alertService,
        \Modules\Core\Services\AttackCorrelationService $correlationService,
        \Modules\Core\Services\FileIntegrityService $fileIntegrityService,
        \Modules\Core\Services\SecurityNotificationService $notificationService,
        \Modules\Core\Services\SecurityAssessmentService $assessmentService
    ) {
    ...
    {
        $ipAddressRaw = $request->input('ip_address', \Modules\Core\Helpers\IpHelper::getClientIp($request));
        $ipAddress = is_string($ipAddressRaw) ? $ipAddressRaw : '';
    ...
    public function clear(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if ($retainDaysRaw) {
                ...
                $countRaw = \Modules\Core\Models\SecurityLog::where('created_at', '<', now()->subDays($retainDays))->delete();
            ...
            \Modules\Core\Models\SecurityLog::truncate();
    ...
    public function fileIntegrityStatus(): \Illuminate\Http\JsonResponse
    {
        ...
        $baselinesCount = \Modules\Core\Models\FileIntegrityBaseline::count();
        $lastIntegrityEvent = \Modules\Core\Models\SecurityLog::whereIn('event_type', [
            'integrity_check',
    ...
    public function resyncFileIntegrity(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $clearedLogs = 0;
            if ($clearHistory) {
                $clearedLogs = \Modules\Core\Models\SecurityLog::whereIn('event_type', [
                    'integrity_check',
                    'file_integrity_violation',
                ])->delete();
            }
            ...
            $user = $request->user();
            \Modules\Core\Models\SecurityLog::log(
                'integrity_baseline_resynced',
                $user,
    ...
    public function autoTuneLogs(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $logs = \Modules\Core\Models\SecurityLog::where('event_type', 'auto_tune')
            ->latest()
            ->paginate($perPage);
    ...
    {
        $service = app(\Modules\Core\Services\SecurityMaintenanceService::class);
    ...
    public function maintenanceActivate(Request $request): \Illuminate\Http\JsonResponse
    {
        $request->validate([
            'modules' => 'array',
            'modules.*' => 'string|in:all,'.implode(',', \Modules\Core\Services\SecurityMaintenanceService::MODULES),
    ...
    {
        $securityRetention = \Modules\Core\Models\Setting::get('security_log_retention_days', 90);
        $activityRetention = \Modules\Core\Models\Setting::get('activity_log_retention_days', 90);
        $loginRetention = \Modules\Core\Models\Setting::get('login_history_retention_days', 180);
        $frequencyRaw = \Modules\Core\Models\Setting::get('security_autotune_frequency', 'weekly');
    ...
    public function updateSettings(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        foreach ($intSettings as $key => $default) {
            if ($request->has($key)) {
                ...
                \Modules\Core\Models\Setting::set($key, $value, 'integer', 'security');
                $updated[$key] = $value;
            }
        }
        ...
        if ($request->has('security_autotune_frequency')) {
            ...
            \Modules\Core\Models\Setting::set('security_autotune_frequency', $freq, 'string', 'security');
