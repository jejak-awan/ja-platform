...
class AuthController extends BaseApiController
{
    ...
    public function login(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        if (\Modules\Core\Models\Setting::get('enable_captcha', false) &&
            \Modules\Core\Models\Setting::get('captcha_on_login', true) &&
        ...
        $ipAddress = \Modules\Core\Helpers\IpHelper::getClientIp($request);
        ...
        if (! $user || ! Hash::check($password, (string) $user->password)) {
            ...
            if ($user) {
                \Modules\Core\Models\LoginHistory::create([
                    'user_id' => $user->id,
        ...
        try {
            if (class_exists(\Modules\Core\Models\ActivityLog::class)) {
                \Modules\Core\Models\ActivityLog::log('login', null, [], $user, 'User logged in');
            }
        } catch (\Exception $e) {
        ...
        $singleSession = \Modules\Core\Models\Setting::get('single_session_enabled', false);
        $maxSessionsRaw = \Modules\Core\Models\Setting::get('max_concurrent_sessions', 0);
        ...
        if ($singleSession) {
            ...
            try {
                ...
                \Modules\Core\Services\SessionManager::setLifetimeForUser($user);
        ...
        $targetUrl = (is_string($baseUrl) ? rtrim($baseUrl, '/') : '').'/dash';
    ...
    public function register(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $registrationEnabled = \Modules\Core\Models\Setting::get('enable_registration', true);
        ...
        try {
            // Verify Captcha
            if (\Modules\Core\Models\Setting::get('enable_captcha', false) && \Modules\Core\Models\Setting::get('captcha_on_register', true)) {
                $captchaService = new \Modules\Core\Services\CaptchaService;
    ...
    public function logout(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        // Log activity
        if ($user) {
            \Modules\Core\Models\ActivityLog::log('logout', null, [], $user, 'User logged out');
        }
    ...
    public function resetPassword(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        // Log activity
        \Modules\Core\Models\ActivityLog::log('password_reset', null, [], $user, 'Password reset via email');
