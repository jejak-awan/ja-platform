...
class ExtensionControllerTest extends TestCase
{
    ...
    {
        // Clean up any uploaded test directories
        $testPluginDir = base_path('Plugins/test-uploaded-plugin');
    ...
    public function test_admin_can_upload_valid_extension_zip(): void
    {
        ...
        $this->assertTrue(is_dir(base_path('Plugins/test-uploaded-plugin')));
        $this->assertTrue(file_exists(base_path('Plugins/test-uploaded-plugin/manifest.json')));
        $this->assertTrue(Extension::where('slug', 'test-uploaded-plugin')->exists());
    }
